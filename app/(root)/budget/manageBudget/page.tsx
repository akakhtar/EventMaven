...
const BudgetDashboard: React.FC = () => {
  ...
  const [eventBudget, setEventBudget] = useState<number>(10000);
 ...
 // State for budget allocation
  ...
  const [eventName, setEventName] = useState('HackathonX 2024');
  ...
  const allocateBudget = () => {
    if (allocatedBudget !== 0) {
      setEventBudget(allocatedBudget+eventBudget);
    }
  };
  ...
  return (
     <div className='container mx-auto p-4'>
      <div className='flex justify-center mb-4'>
              <h1 className="text-2xl font-semibold ">Budget Management for {eventName }</h1>
      </div>
...
<div className="bg-white rounded shadow p-4">
  <h2 className="text-lg font-semibold mb-2">Increase Budget</h2>
  <Input
    type="number"
    className="border border-gray-300 rounded p-2 mb-2"
    placeholder="Enter budget amount"
    value={allocatedBudget === 0 ? 0 : allocatedBudget.toString()} // Convert allocatedBudget to string
    onChange={(e) => setAllocatedBudget(parseInt(e.target.value))}
  />
  <Button onClick={allocateBudget} className="button border rounded p-4 mt-2">Add Budget</Button>
</div>
...
<div className="bg-white rounded shadow p-4">
  ...
  <Button onClick={addExpense} className="button border rounded p-4 mt-2">Add Expense</Button>
